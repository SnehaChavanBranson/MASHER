...
def pre_condition():
    ...
    if object.exists(names.oK_BransonPrimaryButton):
        ClickButton(names.oK_BransonPrimaryButton)
...
class CollapseDistance_Metric:
    def test_collapsedistance_40K_Default(self):
        for row in dataset:
            if testData.field(row, "EXECUTION") == "TRUE" and testData.field(row, "Param_ID") == "COLLAPSE_DISTANCE" and testData.field(row, "UNIT_TYPE") == "METRIC":
                value_40K_default = testData.field(row, "40K_Default")
                if value_40K_default:
                    test.startSection("Weld Mode: Collapse Distance Metric: Test 40K_Default Displayed Value")
    ...
    def test_collapsedistance_40K_min(self):
        for row in dataset:
            if (
                ...
                if value_40K_min:
                    test.startSection("Weld Mode: Collapse Distance Metric: Test 40K_Min")
    ...
    def test_collapsedistance_40K_max(self):
        for row in dataset:
            if (
                ...
                if value_40K_max:
                    test.startSection("Weld Mode: Collapse Distance Metric: Test 40K_Max")
            ...
            if (
                ...
                if value_40K_min and value_40K_max:
                    try:
                        ...
                        continue
                    test.startSection("Weld Mode: Collapse Distance Metric: Test 40K_Mid")
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.startSection("Weld Mode: Collapse Distance Metric: Test 40K_Max + 1 (Out of Range)")
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.startSection("Weld Mode: Collapse Distance Metric: Test 40K_Min - 1 (Out of Range)")
...
class AbsoulteDistance_Metric:
    def test_absoultedistance_40K_Default(self):
        for row in dataset:
            if testData.field(row, "EXECUTION") == "TRUE" and testData.field(row, "Param_ID") == "ABSOLUTE_DISTANCE" and testData.field(row, "UNIT_TYPE") == "METRIC":
                value_40K_default = testData.field(row, "40K_Default")
                if value_40K_default:
                    test.startSection("Weld Mode: Absolute Distance Metric: Test 40K_Default Displayed Value")
                    test.log(f"Testing 40K_Default: {value_40K_default}")
                    try:
                        obj = waitForObject(names.recipeLabWindow_0_00_mm_Text)
                        displayed_value = str(obj.text)
        ...
        for row in dataset:
            if (
                ...
                if not value_40K_min or not value_40K_min.strip():
                    test.log("Skipping row: 40K_Min value is missing or blank.")
                    continue
                test.startSection("Weld Mode: Absolute Distance Metric: Test 40K_Min")
                test.log(f"Testing 40K_Min: {value_40K_min}")
                recipe.AbsoulteDistanceTab_quickedit()    # reset_notification()
                recipe.enterValue(value_40K_min)
                squish.snooze(2)
                recipe.DoneButton()
                test.passes(f"Entered value matches expected min value: {value_40K_min}")
        ...
        for row in dataset:
            if (
                ...
                if not value_40K_max or not value_40K_max.strip():
                    test.log("Skipping row: 40K_Max value is missing or blank.")
                    continue
                test.startSection("Weld Mode: Absolute Distance Metric: Test 40K_Max")
                test.log(f"Testing 40K_Max: {value_40K_max}")
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.fail(f"Invalid numeric values for min: {value_40K_min} or max: {value_40K_max}")
                    continue
                test.startSection("Weld Mode: Absolute Distance Metric: Test 40K_Mid")
                test.log(f"Testing 40K_Mid: {mid_val} (calculated from min={min_val} and max={max_val})")
                recipe.AbsoulteDistanceTab_quickedit()
                recipe.enterValue(str(mid_val))
                squish.snooze(2)
    ...
    def test_absoultedistance_40K_max_plus_one(self):
        for row in dataset:
            if (
                ...
                value_40K_max = testData.field(row, "40K_Max")
                if not value_40K_max or not value_40K_max.strip():    # reset_notification()p():
                    test.log("Skipping row: 40K_Max value is missing or blank.")
                    continue
                try:
                    value_40K_max_plus_one = str(float(value_40K_max) + 1)
                    test.startSection("Weld Mode: Absolute Distance Metric: Test 40K_Max + 1 (Out of Range)")
                    test.log(f"Testing 40K_Max + 1: {value_40K_max_plus_one}")
                    recipe.AbsoulteDistanceTab_quickedit()
                    recipe.enterValue(value_40K_max_plus_one)
                    squish.snooze(2)
    ...
    def test_absoultedistance_40K_min_minus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_min_minus_one = str(float(value_40K_min) - 1)
                    test.startSection("Weld Mode: Absolute Distance Metric: Test 40K_Min - 1 (Out of Range)")
                    test.log(f"Testing 40K_Min - 1: {value_40K_min_minus_one}")
                    recipe.AbsoulteDistanceTab_quickedit()
                    recipe.enterValue(value_40K_min_minus_one)
                    squish.snooze(2)
...
class WeldAmplitude:
    def test_weld_amplitude_40K_Default(self):
        for row in dataset:    # reset_notification()
            if (
                testData.field(row, "EXECUTION") == "TRUE" and
                ...
                value_40K_default = testData.field(row, "40K_Default")
                if value_40K_default:
                    test.startSection("Weld Mode: Weld Amplitude: Test 40K_Default Displayed Value")
                    test.log(f"Testing 40K_Default: {value_40K_default}")
                    try:
    ...
    def test_weld_amplitude_40K_min(self):
        for row in dataset:
            if (
                ...
                value_40K_min = testData.field(row, "40K_Min")
                if value_40K_min:
                    test.startSection("Weld Mode: Weld Amplitude: Test 40K_Min")
                    test.log(f"Testing 40K_Min: {value_40K_min}")
        ...
        for row in dataset:
            if (
                ...
                if not value_40K_max or not value_40K_max.strip():
                    test.log("Skipping row: 40K_Max value is missing or blank.")
                    continue
                test.startSection("Weld Mode: Weld Amplitude: Test 40K_Max")
                test.log(f"Testing 40K_Max: {value_40K_max}")
                recipe.weld_amplitude_quickedit()
                recipe.enterValue(value_40K_max)
                squish.snooze(2)
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.fail(f"Invalid numeric values for min: {value_40K_min} or max: {value_40K_max}")
                    continue
                test.startSection("Weld Mode: Weld Amplitude: Test 40K_Mid")
                test.log(f"Testing 40K_Mid: {mid_val} (calculated from min={min_val} and max={max_val})")
                recipe.weld_amplitude_quickedit()
                recipe.enterValue(str(mid_val))
                squish.snooze(2)
    ...
    def test_weld_amplitude_40K_max_plus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_max_plus_one = str(float(value_40K_max) + 1)
                    test.startSection("Weld Mode: Weld Amplitude: Test 40K_Max + 1 (Out of Range)")
                    test.log(f"Testing 40K_Max + 1: {value_40K_max_plus_one}")
                    recipe.weld_amplitude_quickedit()
                    recipe.enterValue(value_40K_max_plus_one)
                    squish.snooze(2)
                    recipe.weld_amplitude_DoneButton()
                    if object.exists(names.update_WeldRecipe_Parameter_failed_Text):    # reset_notification()arameter_failed_Text):
                        test.passes(f"Entered value rejected as expected: {value_40K_max_plus_one}")
                        core.ClickButton(names.oK_BransonPrimaryButton)
                    else:
                        test.fail("Expected validation popup did not appear.")
    ...
    def test_weld_amplitude_40K_min_minus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_min_minus_one = str(float(value_40K_min) - 1)
                    test.startSection("Weld Mode: Weld Amplitude: Test 40K_Min - 1 (Out of Range)")
                    test.log(f"Testing 40K_Min - 1: {value_40K_min_minus_one}")
                    recipe.weld_amplitude_quickedit()
                    recipe.enterValue(value_40K_min_minus_one)
                    squish.snooze(2)
    ...
    def test_downspeed_40K_Default(self):
        for row in dataset:
            if testData.field(row, "EXECUTION") == "TRUE" and testData.field(row, "Param_ID") == "DOWNSPEED":
                ...
                value_40K_default = testData.field(row, "40K_Default")
                if value_40K_default:
                    test.startSection("Weld Mode: Downspeed: Test 40K_Default Displayed Value")
                    test.log(f"Testing 40K_Default: {value_40K_default}")
                    try:
    ...
    def test_downspeed_40K_min(self):
        ...
        for row in dataset:
            if (
                ...
                value_40K_min = testData.field(row, "40K_Min")
                if value_40K_min:
                    test.startSection("Weld Mode: Downspeed: Test 40K_Min")
                    test.log(f"Testing 40K_Min: {value_40K_min}")
    ...
    def test_downspeed_40K_max(self):
        for row in dataset:
            if (
                ...
                value_40K_max = testData.field(row, "40K_Max")
                if value_40K_max:
                    test.startSection("Weld Mode: Downspeed: Test 40K_Max")
                    test.log(f"Testing 40K_Max: {value_40K_max}")
                    recipe.downspeed_quickedit()
                    recipe.enterValue(value_40K_max)
                    squish.snooze(2)
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.fail(f"Invalid numeric values for min: {value_40K_min} or max: {value_40K_max}")
                    continue
                test.startSection("Weld Mode: Downspeed: Test 40K_Mid")
                test.log(f"Testing 40K_Mid: {mid_val} (calculated from min={min_val} and max={max_val})")
                recipe.downspeed_quickedit()
                recipe.enterValue(str(mid_val))
                squish.snooze(2)    # reset_notification()
                recipe.DoneButton()
                test.passes(f"Entered value matches expected mid value: {mid_val}")
                test.endSection()
    ...
    def test_downspeed_40K_max_plus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_max_plus_one = str(float(value_40K_max) + 1)
                    test.startSection("Weld Mode: Downspeed: Test 40K_Max + 1 (Out of Range)")
                    test.log(f"Testing 40K_Max + 1: {value_40K_max_plus_one}")
                    recipe.downspeed_quickedit()
                    recipe.enterValue(value_40K_max_plus_one)
                    squish.snooze(2)
    ...
    def test_downspeed_40K_min_minus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_min_minus_one = str(float(value_40K_min) - 1)
                    test.startSection("Weld Mode: Downspeed: Test 40K_Min - 1 (Out of Range)")
                    test.log(f"Testing 40K_Min - 1: {value_40K_min_minus_one}")
                    recipe.downspeed_quickedit()
                    recipe.enterValue(value_40K_min_minus_one)
                    squish.snooze(2)
    ...
    def test_weldpressure_40K_Default(self):
        for row in dataset:
            if (
                ...
                value_40K_default = testData.field(row, "40K_Default")
                if value_40K_default:
                    test.startSection("Weld Mode: Weld Pressure: Test 40K_Default Displayed Value")
                    test.log(f"Testing 40K_Default: {value_40K_default}")
                    try:
    ...
    def test_weldpressure_40K_min(self):
        ...
        for row in dataset:
            if (
                ...
                value_40K_min = testData.field(row, "40K_Min")
                if value_40K_min:
                    test.startSection("Weld Mode: Weld Pressure: Test 40K_Min")
                    test.log(f"Testing 40K_Min: {value_40K_min}")
    ...
    def test_weldpressure_40K_max(self):
        for row in dataset:
            if (
                ...
                value_40K_max = testData.field(row, "40K_Max")
                if value_40K_max:
                    test.startSection("Weld Mode: Weld Pressure: Test 40K_Max")
                    test.log(f"Testing 40K_Max: {value_40K_max}")
                    recipe.weld_pressure_quickedit()
                    recipe.enterValue(value_40K_max)
                    squish.snooze(2)
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.fail(f"Invalid numeric values for min: {value_40K_min} or max: {value_40K_max}")
                    continue
                test.startSection("Weld Mode: Weld Pressure: Test 40K_Mid")
                test.log(f"Testing 40K_Mid: {mid_val} (calculated from min={min_val} and max={max_val})")
                recipe.weld_pressure_quickedit()
                recipe.enterValue(str(mid_val))
                squish.snooze(2)
    ...
    def test_weldpressure_40K_max_plus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_max_plus_one = str(float(value_40K_max) + 1)
                    test.startSection("Weld Mode: Weld Pressure: Test 40K_Max + 1 (Out of Range)")
                    test.log(f"Testing 40K_Max + 1: {value_40K_max_plus_one}")
                    recipe.weld_pressure_quickedit()
                    recipe.enterValue(value_40K_max_plus_one)
                    squish.snooze(0.2)
                    recipe.weld_amplitude_DoneButton()
                    squish.snooze(0.2)    # reset_notification()
    ...
    def test_weldpressure_40K_min_minus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_min_minus_one = str(float(value_40K_min) - 1)
                    test.startSection("Weld Mode: Weld Pressure: Test 40K_Min - 1 (Out of Range)")
                    test.log(f"Testing 40K_Min - 1: {value_40K_min_minus_one}")
                    recipe.weld_pressure_quickedit()
                    recipe.enterValue(value_40K_min_minus_one)
                    squish.snooze(2)
    ...
    def test_holdpressure_40K_Default(self):
        for row in dataset:
            if (
                ...
                value_40K_default = testData.field(row, "40K_Default")
                if value_40K_default:
                    test.startSection("Weld Mode: Hold Pressure: Test 40K_Default Displayed Value")
                    test.log(f"Testing 40K_Default: {value_40K_default}")
                    try:
                        displayed_value = str(findObject(names.recipeLabWindow_138_kPa_Text).text)
                        test.log(f"Checking if the default value matches with the actual data")
    ...
    def test_holdpressure_40K_min(self):
        ...
        for row in dataset:
            if (
                ...
                value_40K_min = testData.field(row, "40K_Min")
                if value_40K_min:
                    test.startSection("Weld Mode: Hold Pressure: Test 40K_Min")
                    test.log(f"Testing 40K_Min: {value_40K_min}")
    ...
    def test_holdpressure_40K_max(self):
        for row in dataset:
            if (
                ...
                value_40K_max = testData.field(row, "40K_Max")
                if value_40K_max:
                    test.startSection("Weld Mode: Hold Pressure: Test 40K_Max")
                    test.log(f"Testing 40K_Max: {value_40K_max}")
                    recipe.hold_pressure_quickedit()
                    recipe.enterValue(value_40K_max)
                    squish.snooze(2)
        ...
        for row in dataset:
            if (
                ...
                try:
                    ...
                    test.fail(f"Invalid numeric values for min: {value_40K_min} or max: {value_40K_max}")
                    continue
                test.startSection("Weld Mode: Hold Pressure: Test 40K_Mid")
                test.log(f"Testing 40K_Mid: {mid_val} (calcul    # reset_notification()ated from min={min_val} and max={max_val})")
                recipe.hold_pressure_quickedit()
                recipe.enterValue(str(mid_val))
                squish.snooze(2)
                recipe.DoneButton()
    ...
    def test_holdpressure_40K_max_plus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_max_plus_one = str(float(value_40K_max) + 1)
                    test.startSection("Weld Mode: Hold Pressure: Test 40K_Max + 1 (Out of Range)")
                    test.log(f"Testing 40K_Max + 1: {value_40K_max_plus_one}")
                    recipe.hold_pressure_quickedit()
                    recipe.enterValue(value_40K_max_plus_one)
                    squish.snooze(0.2)
    ...
    def test_holdpressure_40K_min_minus_one(self):
        for row in dataset:
            if (
                ...
                try:
                    value_40K_min_minus_one = str(float(value_40K_min) - 1)
                    test.startSection("Weld Mode: Hold Pressure: Test 40K_Min - 1 (Out of Range)")
                    test.log(f"Testing 40K_Min - 1: {value_40K_min_minus_one}")
                    recipe.hold_pressure_quickedit()
                    recipe.enterValue(value_40K_min_minus_one)
                    squish.snooze(2)
...
def main():
    ...
    pre_condition()
    # reset_notification()
    core.moduleName("Recipe: Weld Mode Metric")
    ...
    #getting error for the object even if its visible
    ...
    holdpressure.test_holdpressure_40K_min()
